...
export function CalendarJuly({ className = "" }: { className?: string }) {
  const days = Array.from({ length: 31 }, (_, d) => d + 1);
  /** July 1, 2026 is Wednesday (Sun-start grid). */
  const startPad = 3;
  return (
    <div
    ...
    >
      <div className="mb-2 text-center text-base font-bold text-violet-500 sm:mb-3 sm:text-lg">
        July 2026
      </div>
        ...
        {days.map((d) => (
          <span
            ...
            className={`rounded-full py-1.5 sm:py-2 ${
              d === 13
                ? "bg-violet-400 font-bold text-white ring-2 ring-violet-600"
                : "text-slate-700"
            }`}
          >
            {d}
...
export function ViewfinderPhoto({
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-md border-[3px] border-neutral-900 bg-black shadow-lg ring-2 ring-violet-200 ${className}`}
    >
